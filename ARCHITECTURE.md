...
- `_assets.py` — makes sure hello_imgui can resolve the FontAwesome font and computes config/`.ini` paths under `$XDG_CONFIG_HOME/imgui_data_loader`. `ensure_assets()` is deliberately non-clobbering: it no-ops when the icon font already resolves and otherwise *adds* imgui-bundle's bundled assets as a search path, so a host app's own `set_assets_folder` choice survives. Only an explicit `assets_folder` replaces the folder. The library ships no fonts of its own.
